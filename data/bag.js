...
function displayBagSummary(){
  ...
  let totalItems = bagArrObjects.length;
  let totalMRP = 0;
  let totalDiscount = 0;

  bagArrObjects.forEach(bagItem => {
...
function loadBagArrObjects(){
    console.log(bagArr);

    bagArrObjects = bagArr.map(itemId => {
...
function displayBagItems (){
    ...
    bagArrObjects.forEach(bagItem => {
      cartItems += generateItemHtml(bagItem);
    });
...
function removeItem(item_Id){
  bagArr = bagArr.filter(bagArrId => bagArrId != item_Id);
